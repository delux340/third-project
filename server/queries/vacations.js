...
const AddVacation = () => {
    return "INSERT INTO `vacations` (`description`, `destination`, `image`, `from`, `until`, `price`) VALUES (?,?,?,?,?,?);"

}

const editVacation = () => {
    return "UPDATE `vacations` SET `description` = ?, `destination` = ?, `image` = ?, `from` = ?, `until` = ?, `price` = ? WHERE (`id` = ?);"

}

const removeVacation = () => {
    return "DELETE FROM `vacations` WHERE (`id` = ?);"
}


const getVacationsFollowers = () => {
    return `SELECT 
vacations.*,(SELECT COUNT(vacation_id) FROM vacations_followers WHERE vacations_followers.vacation_id = vacations.id) AS followers_count,
